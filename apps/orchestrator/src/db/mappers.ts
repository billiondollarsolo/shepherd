...
export function rowToSession(row: AgentSessionRow): SessionRecord {
  return {
    ...
    reasoningEffort: (row.reasoningEffort ?? null) as SessionRecord['reasoningEffort'],
    structuredChat: row.structuredChat ?? false,
...
export function sessionToRow(session: SessionRecord): NewAgentSessionRow {
  return {
    ...
    structuredChat: session.structuredChat ?? false,
