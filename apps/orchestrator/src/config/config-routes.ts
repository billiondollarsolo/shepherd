...
export function registerConfigRoutes(app: FastifyInstance, svc: ConfigService, authDeps: AuthGuardDeps): void {
  const requireAuth = makeRequireAuth(authDeps);

  app.post('/api/config/apply', { preHandler: requireAuth }, async (req: FastifyRequest, reply: FastifyReply) => {
    const body = (req.body ?? {}) as { yaml?: string };
    if (typeof body.yaml !== 'string' || body.yaml.trim().length === 0) {
      return reply.code(400).send({ error: { code: 'bad_request', message: 'yaml is required' } });
    }
    try {
      ...
      return reply.code(500).send({ error: { code: 'internal', message: 'config apply failed' } });
    }
  });

  app.get('/api/config/export', { preHandler: requireAuth }, async (_req: FastifyRequest, reply: FastifyReply) => {
