...
export interface AuthScreenProps {
  initialMode: Mode;
  /** Production fresh installs require the out-of-band secret generated by the operator. */
  ...
  async function onSubmit(e: FormEvent): Promise<void> {
    ...
    if (mode === 'setup') {
      if (setupTokenRequired && !setupToken.trim()) {
        setError('Enter the setup token generated on the Shepherd server.');
        return;
      }
    ...
    try {
      ...
      if (err instanceof ApiError && err.code === 'invalid_setup_token') {
        setError('The setup token is invalid. Read the current token from the Shepherd server.');
    ...
    >
      ...
      <section className="relative flex min-w-0 flex-1 flex-col justify-center px-4 py-8 sm:px-10 sm:py-12 lg:px-14">
        ...
        <div className="relative mx-auto w-full min-w-0 max-w-[22rem]">
          <TransportWarning warning={transportWarning} />
          <div className={`${transportWarning ? 'mt-6' : ''} mb-8 lg:mb-10`}>
            <BrandWordmark className={`${transportWarning ? 'mb-5' : 'mb-8'} lg:hidden`} />
            ...
            {isSetup && setupTokenRequired ? (
              <div className="grid gap-1.5">
                ...
                <p className="text-2xs text-flock-ink-muted">
                  Read it from <code>secrets/setup_token</code> on the Shepherd server.
                </p>
              </div>
            ) : null}
            <div className="grid gap-1.5">
              ...
              <Input
                ...
                required
                autoFocus={!isSetup || !setupTokenRequired}
