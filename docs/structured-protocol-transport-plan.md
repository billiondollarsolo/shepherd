...
**Chosen scope (confirmed):** _Add a structured-protocol transport; keep the
platform._ Not a rewrite; not a fork.
...
1. **agentd already ships a complete, unit-tested ACP JSON-RPC client in Go**
   ...
   is the _orchestrator's_ choice to select ACP (`acpLaunchCommand` returns `null`),
...
3. **Effect footprint is a liability:** t3code pins `effect@4.0.0-beta.78` (pre-1.0
   beta) _with a local patch_, on `effect/unstable/rpc`. Not worth importing for a
...
**What we take from t3code:** protocol _semantics_ as a reference, and the fact that
