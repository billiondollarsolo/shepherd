...
**Project:** Flock — a web cockpit for supervising a *flock* of CLI coding agents
...
> testing) on top of `PRD.md`. The PRD remains the authoritative source for *product
> intent*; this spec is the authoritative source for *how we build it*. Every FR-/NFR-
...
*you*." Flock solves persistence, unified status, away alerts, and per-session browser
...
- Native mobile app (API is native-*ready* only).
...
*As a builder, I need to know whether wterm exposes OSC 9/777 + BEL handlers so I can pick the terminal emulator.*
...
*As a builder, I need to resolve the browser-harness gates before relying on it for Layer B.*
